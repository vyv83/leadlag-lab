...
# Explorer
explorer.html?session={session_id}                  ← все события сессии
explorer.html?session={session_id}&event={bin_idx}  ← конкретное событие
explorer.html?session={session_id}&event={bin_idx}&mode=trade&backtest={bt_id}&trade={n}
...
# Data Quality
quality.html?session={session_id}                   ← качество данных сессии
```
...
- Run Analysis (выбрать сессию сбора)
...
URL: `explorer.html?session={session_id}` или `explorer.html?session={session_id}&event={bin_idx}&mode=trade&backtest={bt_id}&trade={n}`
...
- Session: dropdown (если несколько сессий анализа)
...
- Кнопка "View Event" → `explorer.html?session={session_id}&event={bin_idx}`
...
- Exit reason: hold_expired / stop_loss / take_profit
- Кнопки: ← Prev, Next →
- Кнопка: "View in Explorer" → `explorer.html?session={id}&event={bin_idx}`
...
URL: `quality.html?session={session_id}`

**Сводная таблица по venues (для выбранной сессии)**
...
- `GET /api/collector/status` → `{running, session_id, start_time, planned_duration_s, venues: [{name, status, ticks, bbo, reconnects, last_tick_ts, last_error, uptime_pct}]}`
- `POST /api/collector/start` body: `{venues: [], duration_s, bin_size_ms, rotation_s}` → `{session_id}`
...
- `GET /api/sessions` → `[{id, collection_id, params_hash, date, duration_h, n_ticks, n_events, n_venues}]`
- `GET /api/sessions/{id}/meta` → `{params, time_range, venues, quality_summary}`
- `GET /api/sessions/{id}/events` query: `?signal=C&min_mag=2.0&follower=Lighter&direction=1&min_lagging=3` → `[{event with all metrics, БЕЗ ценовых окон}]`
- `GET /api/sessions/{id}/event/{bin_idx}` → `{event + price_window (из price_windows.json) + bbo_window (из bbo_windows.json) для графика}`
- `GET /api/sessions/{id}/quality` → `{venue quality metrics}`
- `POST /api/sessions/{id}/analyze` body: `{params}` → `{ok, session_id (с params_hash), events_count}`
...
- `GET /api/backtests` → `[{id, strategy, session, date, summary_stats}]`
- `GET /api/backtests/{id}/meta` → `{strategy, session, params, params_override, slippage_model, entry_type}`
...
- `POST /api/backtests/run` body: `{strategy_name, session_id, params_override: {key: value} | null}` → `{backtest_id}`
...
- Analysis — не daemon, запускается как subprocess из API при `POST /api/sessions/{id}/analyze`
...
5. ДЕТАЛЬНЫЙ ПРОСМОТР В ПРИЛОЖЕНИИ
   ...
   Кнопка "View Event" → explorer.html?session={id}&event={bin_idx}
...
> Создай файл `ui/explorer.html`. На вход: JSON структура из `data/sessions/{id}/events.json` (формат описан ниже) и `data/sessions/{id}/price_windows.json` + `data/sessions/{id}/bbo_windows.json`. Events загружаются через `fetch('/api/sessions/{id}/events')` при старте страницы. Price/BBO windows загружаются через `fetch('/api/sessions/{id}/event/{bin_idx}')` при клике на конкретное событие (lazy loading).
> 
...
> 
> Критерий приёмки: открывается в браузере, 500 событий загружаются за < 2 секунд, фильтрация мгновенная, график перерисовывается при клике, BBO spread виден на нижнем субплоте. URL с ?session=X&event=Y открывает нужное событие.
...
- Критерий: `explorer.html?session=X` загружает 519 событий за < 2 секунд, фильтры работают мгновенно, клик на событие перерисовывает график за <200ms, BBO overlay виден на нижнем субплоте для venue с BBO, скрыт для venue без BBO
...
- Критерий: `quality.html?session=X` показывает красные флаги для Aster/Binance, BBO analysis с медианными спредами, колонку BBO available
